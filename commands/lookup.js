  ...
  async execute(interaction) {
      try {
          ...
          if (interaction.options.getSubcommand() === 'player') {
              ...
              let hackerData1 = await hackerDB.findOne({
                  gamertag: query
              })
                  ...
                  .then(response => {
                      XboxLiveAPI.getPlayerSettings(
                          ...
                          return await interaction.editReply({
                              content: `Invalid Query! I couldn't find that player!`,
                              ephemeral: true
                          })
                      })
                  })
          ...
          if (interaction.options.getSubcommand() === 'realm') {
              ...
              const realm = await api.getRealmFromInvite(`${query}`).catch(async error => {
                  console.log(error)
                  await interaction.editReply({
                      content: `Invalid Query! I couldn't find that realm!`,
                      ephemeral: true
                  })
                  return
              })
              var realmData = await realmProfileDB.findOne({
                  profileID: realm.name
              })
              if (realmData != null) {
                  var realmProfileID = realmData.profileID
                  var hackerCount = realmData.hackerCount
              }
              if (!realmData) var realmProfileID = 'N/A';
              var hackerCount = 'N/A'
