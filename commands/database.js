...
const {
  authenticate
} = require('@xboxreplay/xboxlive-auth')
  ...
  async execute(interaction) {
      try {
              ...
              if (databaseType === 'discord') {
                  ...
                  if (submitted) {
                      ...
                      await submitted.reply({
                          content: `<:yes:1070502230203039744> Successfully added **${user.tag}・${discordID}** to the Discord User Database!`,
                          ephemeral: true
                      })
                  }
              }
          ...
          if (interaction.options.getSubcommand() === 'search') {
              ...
              if (databaseType === 'realm') {
                  ...
                  if (submitted) {
                      ...
                      if (!gamertag && !discordid && !realm && !profileid) return await submitted.reply({
                          content: `You must choose atleast one option!`,
                          ephemeral: true
                      })
                      if (gamertag) {
                          ...
                          if (!hackerProfile) return await submitted.reply({
                      ...
                      if (realm) {
                          ...
                          if (!hackerProfile) return await submitted.reply({
                              content: `Realm not found in the database!`,
                              ephemeral: true
                          })
                          let realmProfile = await realmProfileDB.findOne({
                              name: realm
                          })
                          searchEmbed = {
                              color: 946466,
                      ...
                      if (profileid) {
                          ...
                          if (!realmProfile) return await submitted.reply({
                              content: `Realm not found in the database!`,
                              ephemeral: true
                          })
                          searchEmbed = {
              ...
              if (databaseType === 'discord') {
                  ...
                  await interaction.showModal(searchModal).catch((error) => {
                      return console.log(error)
                  })
                  const submitted = await interaction.awaitModalSubmit({
                      time: 99999,
                  ...
                  if (submitted) {
                      ...
                      if (!user) return await submitted.reply({
                          content: `<:error:1086371516565950474> **IdError:** User not found!`,
                          ephemeral: true
                      })
                      if (!hackerProfile) return await submitted.reply({
          ...
          await interaction.deferReply({
              content: `Loading <a:loading:1069385540077637742>`
          });
          if (interaction.options.getSubcommand() === 'leaderboard') {
              ...
              if (interaction.options.getString('type') === 'admin') {
                  if (!userData.isAdmin) return await interaction.editReply({
                      content: `Invalid Permission! You must be a Realms+ Admin to run this command!`,
                      ephemeral: true
                  })
