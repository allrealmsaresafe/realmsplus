...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (userData.botBan) return interaction.reply({
        content: `Uh oh! You are banned from using Realms+!`,
        ephemeral: true
    })
    if (commandCooldown.has(interaction.user.id)) {
        return await interaction.reply({
            content: `You can only run a command every 5 seconds!`,
            ephemeral: true
        });
    } else {
        if (!userData.isAdmin) {
        ...
        try {
            ...
            const errorChannel = client.channels.cache.get(`${process.env.ERROR_CHANNEL}`)
