...
exports.run = async (message, args) => {
    if (args.toString().replaceAll(' ', '') === '') return message.reply(`\`!botban\` is a command that when executed on a user, bans them from inviting and using RealmDB.\n\nSyntax: !botban <user-id>`)
    ...
    const reportLog = {
      ...
      message.reply(`Successfully banned <@${args.toString().replaceAll(' ', '')}> from using RealmDB!`)
